...
public class GenAutoChooser {
    ...
    private void setRoutine(Drive drive, Superstructure superstructure, Intake intake){
        ...
        cmds.add(
            ree[0].charAt(0) == 'c'?
                new coralScore(
                    getCoralGoalPos(ree[0].charAt(2), ree[0].charAt(1)), 
                    Character.getNumericValue(ree[0].charAt(3)), 
        ...
        for(int i = 1; i < ree.length; i ++){
            switch(ree[i].charAt(0)){ //algae or coral
                case 'c':
                    cmds.add(new coralSource(source.get(), drive, superstructure, intake));
                    cmds.add(new coralScore(
                        getCoralGoalPos(ree[i].charAt(2), ree[i].charAt(1)), 
                        Character.getNumericValue(ree[i].charAt(3)), 
                        drive, superstructure,intake));
                break;
